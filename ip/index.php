...
{
    /*高德地图开发者key*/
    private $key = 'bd3118c94a3183c85b802dd85dbba67c';
    ...
    public function setInfo()
    {
        ...
        imagettftext($this->drawIm, $time_size, $time_angle, $time_x, $time_y, $time_color, $time_font, date('Y年n月j日'));
    ...
    public function picInit()
    {
        ...
        $dst_path = $this->path . 'icon/bg.png';
        //传递背景图片
    ...
    public function setWeather()
    {
        ...
        imagettftext($this->drawIm, 14, 0, 80, 140, imagecolorallocate($this->drawIm, 0, 51, 78), $this->path . "font/msyh.ttf", $this->weather['weather']); //今天天气添加到图片
        imagettftext($this->drawIm, 12, 0, 150, 90, imagecolorallocate($this->drawIm, 0, 51, 78), $this->path . "font/msyh.ttf", '温度:' .  $this->weather['temp'] . '℃'); //当前温度添加到图片
        imagettftext($this->drawIm, 12, 0, 237, 90, imagecolorallocate($this->drawIm, 0, 51, 78), $this->path . "font/msyh.ttf", ' 湿度:' .  $this->weather['humidity'] . '%RH'); //当前湿度添加到图片
        imagettftext($this->drawIm, 12, 0, 150, 110, imagecolorallocate($this->drawIm, 0, 51, 78), $this->path .  "font/msyh.ttf", '风向:' .  $this->weather['winddirection'] . '方'); //当前风向添加到图片
        imagettftext($this->drawIm, 12, 0, 237, 110, imagecolorallocate($this->drawIm, 0, 51, 78), $this->path . "font/msyh.ttf", ' 风力级别:' . $this->weather['windpower'] . '级'); //风力级别添加到图片
        imagettftext($this->drawIm, 12, 0, 150, 130, imagecolorallocate($this->drawIm, 0, 51, 78), $this->path . "font/msyh.ttf", '更新时间:' . $this->weather['reporttime']); //更新时间添加到图片
    ...
    public function getBrowser()
    {
        ...
        if (preg_match('#(Camino|Chimera)[ /]([a-zA-Z0-9.]+)#i', $ua)) {
            $browser = 'Camino';
        } elseif (preg_match('#SE 2([a-zA-Z0-9.]+)#i', $ua)) {
            $browser = '搜狗';
        } elseif (preg_match('#360([a-zA-Z0-9.]+)#i', $ua)) {
            $browser = '360';
        } elseif (preg_match('#QQ/([a-zA-Z0-9.]+)#i', $ua)) {
            $browser = 'QQ内置';
        } elseif (preg_match('#Maxthon( |\/)([a-zA-Z0-9.]+)#i', $ua)) {
            $browser = 'Maxthon';
        } elseif (preg_match('#Chrome/([a-zA-Z0-9.]+)#i', $ua)) {
            $browser = 'Chrome';
        } elseif (preg_match('#Safari/([a-zA-Z0-9.]+)#i', $ua)) {
            $browser = 'Safari';
        } elseif (preg_match('#opera mini#i', $ua)) {
            $browser = 'Opera Mini';
        } elseif (preg_match('#Opera.([a-zA-Z0-9.]+)#i', $ua)) {
            $browser = 'Opera';
        } elseif (preg_match('#(j2me|midp)#i', $ua)) {
            $browser = "J2ME/MIDP Browser";
        } elseif (preg_match('/GreenBrowser/i', $ua)) {
            $browser = 'GreenBrowser';
        } elseif (preg_match('#TencentTraveler ([a-zA-Z0-9.]+)#i', $ua)) {
            $browser = '腾讯TT';
        } elseif (preg_match('#UCWEB([a-zA-Z0-9.]+)#i', $ua)) {
            $browser = 'UCWEB(' . $matches[1];
        } elseif (preg_match('#MSIE ([a-zA-Z0-9.]+)#i', $ua)) {
            $browser = 'Internet Explorer';
        } elseif (preg_match('#avantbrowser.com#i', $ua)) {
            $browser = 'Avant Browser';
        } elseif (preg_match('#PHP#', $ua)) {
            $browser = 'PHP';
        } elseif (preg_match('#danger hiptop#i', $ua)) {
            $browser = 'Danger HipTop';
        } elseif (preg_match('#Shiira[/]([a-zA-Z0-9.]+)#i', $ua)) {
            $browser = 'Shiira';
        } elseif (preg_match('#Dillo[ /]([a-zA-Z0-9.]+)#i', $ua)) {
            $browser = 'Dillo';
        } elseif (preg_match('#Epiphany/([a-zA-Z0-9.]+)#i', $ua)) {
            $browser = 'Epiphany';
        } elseif (preg_match('#UP.Browser/([a-zA-Z0-9.]+)#i', $ua)) {
            $browser = 'Openwave UP.Browser ';
        } elseif (preg_match('#DoCoMo/([a-zA-Z0-9.]+)#i', $ua)) {
            $browser = 'DoCoMo';
        } elseif (preg_match('#(Firefox|Phoenix|Firebird|BonEcho|GranParadiso|Minefield|Iceweasel)/([a-zA-Z0-9.]+)#i', $ua)) {
            $browser = 'Firefox';
        } elseif (preg_match('#(SeaMonkey)/([a-zA-Z0-9.]+)#i', $ua)) {
            $browser = 'Mozilla SeaMonkey';
        } elseif (preg_match('#Kazehakase/([a-zA-Z0-9.]+)#i', $ua)) {
            $browser = 'Kazehakase';
        } else {
            $browser = '未知';
        }
